...
public class MainActivity extends AppCompatActivity {
    ...
    private void enableWifi() {
        // Get a handle to the system-level service WIFI_SERVICE.
        WifiManager wifiManager = (WifiManager) getApplicationContext().getSystemService(WIFI_SERVICE);
        // Check if WiFi is not enabled.
        if(wifiManager != null && !wifiManager.isWifiEnabled()) {
            // It is not possible to enable or disable Wi-Fi programmatically for
            // Android API greater than level 29.
            // Instead use an intent to show the WiFi setting windows.
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) { // Android10, API 29
                Intent intent = new Intent(Settings.Panel.ACTION_INTERNET_CONNECTIVITY);
                startActivity(intent);
            } else {
                try {
                    if(wifiManager.setWifiEnabled(true)) {
                        Log.d(TAG, "Wifi was enabled");
                    } else {
